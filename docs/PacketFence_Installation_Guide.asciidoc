...
WARNING: This feature is not supported on Debian 8, it will be on Debian 9.

You will first need to install OpenVAS 9 along with:

* openvas9-manager
* openvas9-scanner

on a server in order to manage OpenVAS remotely from PacketFence via the `omp` command line.
...
The output of the above command should provide you the version of the OpenVAS Manager. Otherwise, ensure all the necessary components are in place for management through the `omp` command line client and that PacketFence is able to communicate with OpenVAS on port 9390.
...
image::rapid7/rapid7-pf-1.png[scaledwidth="100%",alt="Rapid7 PacketFence config"]

image::rapid7/rapid7-pf-2.png[scaledwidth="100%",alt="Rapid7 PacketFence config"]

image::rapid7/rapid7-pf-3.png[scaledwidth="100%",alt="Rapid7 PacketFence config"]
...
Now that PacketFence is properly configured to receive information from Nexpose, we can configure it to perform some actions on the alerts it receives. In the PacketFence GUI, go to *Configuration -> Compliance -> Violations* and create a new violation.

Make sure you set the following parameters in the 'Definition' tab:
[options="compact"]
* Enable: Set it to *ON* 
* Action: This is where you put what you want PacketFence to do, refer to the violations documentation in this guide for details on these.
...
image::rapid7/rapid7-violation-trigger.png[scaledwidth="100%",alt="Rapid7 violation trigger"]

For more info on violation actions, go to the _Blocking malicious activities with violations_ section of this guide.
...
Using the `report.conf` configuration file, you can define reports that create SQL queries to view tables in the PacketFence database. These reports will appear under the 'Reports -> Custom Reports' menu of the administration interface.
...
[options="compact"]
* * `type`: Determines what type of report this is. Setting `type=built-in` will make this report appear in "Other reports", omitting it will display it in "Custom reports" when viewing it in the administration interface. When in doubt, simply omit this parameter.
